...
export default async function NewArrivals() {
    ...
    try {
        newArrivals = await getNewArrivals(6);
    } catch (error) {
        ...
        >
            ...
            <div className="relative aspect-square overflow-hidden rounded-2xl bg-[#F6F6F6] transition-all duration-500 group-hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)]">
                ...
                <div className="absolute inset-x-4 bottom-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                    <div className="w-full py-4 bg-black text-white rounded-xl shadow-xl flex items-center justify-center gap-2 cursor-pointer">
            ...
            <div className="mt-5 px-1">
                ...
                {/* Product Name */}
                <h3 className="text-sm font-bold text-gray-900 mb-3 leading-snug group-hover:text-[#1e3a8a] transition-colors line-clamp-2 min-h-[2.5rem]">
                    {product.name}
                </h3>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="p-1.5 bg-gray-50 rounded-lg group-hover:bg-[#D4AF37]/10 transition-colors">
                        <Plus className="w-3 h-3 text-gray-400 group-hover:text-[#D4AF37]" />
                    </div>
                </div>
            </div>
        </Link>
