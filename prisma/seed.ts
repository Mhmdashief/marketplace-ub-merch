...
const SUPER_ADMIN_EMAIL    = process.env.SUPER_ADMIN_EMAIL    ?? "superadmin@ubmerch.ac.id";
const SUPER_ADMIN_PASSWORD = process.env.SUPER_ADMIN_PASSWORD ?? "UBMerch@SuperAdmin2026!";
...
async function main() {
  ...
  if (existing) {
    console.log(`✅ SUPER_ADMIN sudah ada: ${existing.email}`);
    console.log("   Seed dilewati untuk menghindari duplikasi.\n");
    return;
  }
